...
class V2stacksApi(object):
    ...
    def change_image(self, name, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.change_image(name, callback=callback_function)
        ...
        if kwargs.get('callback'):
            return self.change_image_with_http_info(name, **kwargs)
        else:
            (data) = self.change_image_with_http_info(name, **kwargs)
            return data

    def change_image_with_http_info(self, name, **kwargs):
        """
        update stack by name
        Stacks are template instances - a running cloud infrastructure created based on a template. Stacks are always launched on behalf of a cloud user account. Stacks support a wide range of resources, allowing you to build a highly available, reliable, and scalable infrastructure for your application needs.
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.change_image_with_http_info(name, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method change_image" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'name' is set
        if ('name' not in params) or (params['name'] is None):
            raise ValueError("Missing the required parameter `name` when calling `change_image`")
        ...
        return self.api_client.call_api('/v2/stacks/changeImage/{name}', 'PUT',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
