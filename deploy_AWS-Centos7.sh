...
sudo curl -sSL https://raw.githubusercontent.com/seanorama/ambari-bootstrap/master/ambari-bootstrap.sh | sudo -E sh
...
export host_count=1
curl -ssLO https://github.com/seanorama/ambari-bootstrap/archive/master.zip; unzip master.zip; cd ambari-bootstrap-master/deploy
# Hack to fix bug in Ambari Blueprints
sudo sed -i.bak "s/\(^    total_sinks_count = \)0$/\11/" /var/lib/ambari-server/resources/stacks/HDP/2.0.6/services/stack_advisor.py
# Blueprint
curl -sSL https://gist.githubusercontent.com/Chaffelson/f699d5f5fbb6146e1a9702cffa5c0cc0/raw/acf67a23e017f2cd304ea41fbe4ef402fc6c35c6/configuration-custom.json > configuration-custom.json
# This command might fail with 'resources' error, means Ambari isn't ready yet
sleep 10
./deploy-recommended-cluster.bash
echo Now open your browser to http://$(curl -s icanhazptr.com):8080 to observe the cluster install